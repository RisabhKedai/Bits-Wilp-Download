  ...
  try {
    ...
    if (cookies) {
      const validCookies = cookies.filter(
        (cookie) => !cookie.expires || cookie.expires < Date.now(),
      );
...
async function checkBitsLoggedIn(page) {
  ...
  return (
    myCoursesLink.length > 0 && myCoursesLink.text().trim() === "My Courses"
  );
}
...
async function checkTaxilaLoggedIn(page) {
  const $ = cheerio.load(await page.content());
  const logoutLink = $(
    'a.dropdown-item.menu-action[role="menuitem"][data-title="logout,moodle"]',
  );
